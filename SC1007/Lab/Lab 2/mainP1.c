...
void reverseStack(Stack *sPtr)
{
    ...
    Stack *original = sPtr;
    ListNode *og = original->head;
    printf("Original List has %d elements.\n", original->size);
    while (og != NULL)
    {
        item = og->item;
        push(&newStack, item);
    ...
    printf("newStack head pointer = %p\n",newStack.head);
    printf("newStack has %d elements\n",newStack.size);
    *sPtr = newStack; 
}

/*
This question was solved through pushing elements into a new stack
using the values from the original stack. This flips the order.
Once done we just need to change the ptr address to the new stack.
*/
