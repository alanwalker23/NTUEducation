...
int removeBSTNode(BTNode **nodePtr, int item)
{
    ...
    BTNode *current = *nodePtr;
    BTNode *temp;
    if (current==NULL)
        return 0;
    if(current->item>item)
        return removeBSTNode(&current->left,item);
    else if (current->item<item)
        return removeBSTNode(&current->right,item);
    else
    {
        //Current item is == item;
        if(current->left!=NULL && current->right!=NULL)
        {
            //Start with left (smaller) then going down it's right till null.
            for(temp=current->left;temp->right!=NULL;temp=temp->right);
            current->item = temp->item; //Copy successor (replace current item)
            return removeBSTNode(&current->left,temp->item);
        }
        else
        {
            temp=current;
