...
int SD(Graph g, int v, int z)
{
    ...
    while (!isEmptyQueue(q1) && found==0)
    {
        ...
        if (isEmptyQueue(q1) && found==0)
        {
            //printf("Originally q1 is %d while q2 is %d\n",q1.size,q2.size);
            q1 = q2;
            removeAllItemsFromQueue(&q2);
            q2.head = NULL;
            q2.size = 0;
            //printf("Q1 is now %d Q2 size is now %d\n",q1.size,q2.size);
            layers++;
        }
    }
    if (found==1)
