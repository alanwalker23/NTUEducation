...
void BuildList(BTNode *root)
{
    ...
    {
        if (current->right->item < current->left->item)
        {
            // printf("Happen\n");
            int temp = current->right->item;
            current->right->item = current->left->item;
            current->left->item = temp;
        }
    }
    if (current->left != NULL)
        BSTCorrection(current->left);
    // printf("Value is %d\n", current->item);
    if (current->right != NULL)
        BSTCorrection(current->right);
}