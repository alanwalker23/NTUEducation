    ...
    public static int[] dijkstra(int V, ArrayList<ArrayList<AdjListNode>> graph, int src) {
        ...
        while (pq.size() > 0) {
            ...
            for (AdjListNode n : graph.get(current.getVertex())) {
                System.out.println("Current vertex is : " + current.getVertex());
                System.out.printf("n vertex is: %d \tn weight is: %d\n", n.getVertex(), n.getWeight());
