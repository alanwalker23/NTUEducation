...
void createTable(Rule *table, int *size)
{
    ...
    *size = 0;
    ...
    for (int i = 0; i < *size; i++)
    {
        ...
        scanf("%c", &newRule.source);
        getchar();
        printf("Enter code character:\n");
        scanf("%c", &newRule.code);
        getchar();
        printf("Source - %c Code - %c\n", newRule.source, newRule.code);
        table[i] = newRule;
    }
}
void encodeChar(Rule *table, int size, char *s, char *t)
{
    ...
    for (int j = 0; j < strlen(s); j++)
    {
        for (int i = 0; i < size; i++)
        {
            Rule currentRule = table[i];
            //printf("Current letter %c \n",s[j]);
            //printf("Current source %c \n",currentRule.source);
            //printf("Current code %c \n",currentRule.code);
            if (s[j] == currentRule.source)
            {
                s[j] = currentRule.code;
                break;
            }
        }
    }
